...
export const getCalculationFarm = (groupedArray , propertiesArray) => {
    ...
    for (let index = 0; index < propertiesArray.length; index++)
    {
        ...
        let CatAgeArr =arrIsNotEmtyOrUndefined(filteredCAts) === true?
            filteredCAts.map((x) => toAge(x.datumNarodenia)) : undefined;
        let DogAgeArr = arrIsNotEmtyOrUndefined(filteredDogs) === true?
            filteredDogs.map((x)=> toAge(x.datumNarodenia)) : undefined;

        let item = {
            Id: +element,
            CountOfCats: ownerGroup?.filter((x) => x.type === PetType.CAT).length,
            CountOfDogs: ownerGroup?.filter((x) => x.type === PetType.DOG).length,
            SumOfCatsAge: arrIsNotEmtyOrUndefined(CatAgeArr) === true?
                CatAgeArr.reduce((sum, record) => sum + record) : 0,
            SumOfDogsAge: arrIsNotEmtyOrUndefined(DogAgeArr) === true?
                DogAgeArr.reduce((sum, record) => sum + record) : 0
        } ;
        ResultArr.push(item);
    }
    return ResultArr;
